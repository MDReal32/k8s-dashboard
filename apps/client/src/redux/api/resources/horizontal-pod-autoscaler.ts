...
export const horizontalPodAutoscalerApi = createApi({
  reducerPath: `api.k8sd.${ResourceTypes.HORIZONTAL_POD_AUTOSCALER}`,
  baseQuery: fetchBaseQuery({
    baseUrl: new URL(
      `/api/k8s/v1/${ResourceTypes.HORIZONTAL_POD_AUTOSCALER}`,
  ...
  endpoints(builder) {
    return {
      listAll: builder.query<
        Response<ResourceTypeMap[ResourceTypes.HORIZONTAL_POD_AUTOSCALER][]>,
        string
      >({
