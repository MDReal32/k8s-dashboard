...
export const clusterRoleBindingApi = createApi({
  reducerPath: `api.k8sd.${ResourceTypes.CLUSTER_ROLE_BINDING}`,
  baseQuery: fetchBaseQuery({
    baseUrl: new URL(
      `/api/k8s/v1/${ResourceTypes.CLUSTER_ROLE_BINDING}`,
  ...
  endpoints(builder) {
    return {
      listAll: builder.query<
        Response<ResourceTypeMap[ResourceTypes.CLUSTER_ROLE_BINDING][]>,
        string
      >({
        query: namespace => `?namespace=${namespace}`
      }),
      get: builder.query<Response<ResourceTypeMap[ResourceTypes.CLUSTER_ROLE_BINDING]>, GetBody>({
